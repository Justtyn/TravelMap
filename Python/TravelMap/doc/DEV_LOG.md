...
## 七、官网与素材更新
1. **站点文案同步**
   - `templates/home.html` 已替换为真实产品描述：突出“首页/商城/预订/我的”四大模块，并强调收藏、去过、购物车与订单的串联能力。
   - Hero 区域 CTA、功能矩阵与 FAQ 段落仍沿用原有布局，仅更新了文案与数字，确保 App 与官网信息一致。
2. **截图素材**
   - `static/首页景点列表.jpg`：首页 Feed 列表 + 搜索交互。
   - `static/预定页面.jpg`：酒店/票务预订卡片与筛选状态。
   - `static/商城页面.jpg`：商城 Tab 商品流、收藏入口。
   - `static/景点详情页.jpg`：景点详情页含收藏、去过控件。
   - `static/我的页.jpg`：个人中心入口与快捷菜单。
   - `static/我的收藏页.jpg`：收藏 Tab 切换示意。
   - `static/我的去过页.jpg`：去过历史记录与评分展示。
   - `static/我的订单页面.jpg`：订单列表卡片样式。
   - `static/我的购物车页.jpg`：购物车条目与下单按钮。
   - `static/登录页.jpg` / `static/注册页.jpg`：认证流程 UI。
   - `static/深色模式适配.jpg`：暗色主题效果。
