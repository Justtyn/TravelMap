...
说明:
    - 使用 Flask app.test_client()，不依赖外部服务。
    - 会注入部分示例数据（景点 / 商品 / 用户）以便测试。
"""
import json
import random
import sqlite3
...
def main():
    ...
    scenic_items = scenic_resp.get('data') or []
    ...
    p('SCENIC_DETAIL_404', client.get(f'/api/scenics/{(scenic_id or 0) + 9999}'))
    ...
    product_items = product_resp.get('data') or []
    ...
    p('PRODUCT_DETAIL_404', client.get('/api/products/99999'))
    ...
    if booking_hotel.get('data'):
        assert booking_hotel['data'][0]['product']
    booking_ticket = p('BOOKING_TICKET', client.get('/api/bookings?type=TICKET'))
    assert len(booking_ticket.get('data') or []) >= 1
    ...
    p('PLAN_DETAIL_404', client.get('/api/plans/99999'))
